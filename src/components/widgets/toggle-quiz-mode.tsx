...
export default function ToggleQuizMode({ styles, setTextMode, textMode }: IProps) {
    const t = useTranslations('creatingQuiz.questionsPage.toggleMode')
    ...
    return (
        <div className={styles.quiz_mode}>
            <button className={`${textMode ? styles.actived : ''}`}
                onClick={(e) => { e.preventDefault(); setTextMode(true) }}>
                {t('textOnly')}
            </button>
            <button className={`${!textMode ? styles.actived : ''}`}
                onClick={(e) => { e.preventDefault(); handleSetImageMode() }}>
                {t('textAndImage')}
            </button>
            {showPopup && <>
                <PopupWarningGamepass closePopup={() => setShowPopup(false)} />
