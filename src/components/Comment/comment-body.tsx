...
export default function CommentBody({ styles, comment, quizId }: IProps) {
    ...
    const handleEdit = () => { setEditing(!editing); setCommentValueEdited(comment.body) }
    const handleReply = () => setReplying(state => !state)
    const handleShowReplies = () => setShowReplies(state => !state)

    const handleRemove = () => {
        const data = { comment: comment.body }
        deleteComment(quizId, comment.commentId, data).then(res => { if (res?.err) setError(res.err) })
    }
    ...
    const handleSubmitEdit = (e: FormEvent) => {
        ...
        const data = { comment: { body: commentValueEdited, quizId, commentId: comment.commentId } }
        editComment(quizId, comment.commentId, data).then(res => {
            if (res?.err) setError(res.err)
        }).finally(() => { setEditing(false); setLoadingEdit(false) })
    }
    ...
    const handleLikeComment = () => {
        ...
        if (liked) {
            ...
            dislikeComment(comment.commentId).then(res => { if (res?.err) setError(res.err) })
        } else {
            setLiked(true)
            setLikeCount(state => (state ?? 0) + 1)
            likeComment(comment.commentId).then(res => { if (res?.err) setError(res.err) })
        }
    }
    ...
    return (
        <>
            <div className={styles.content_comment}>
                {!editing && <TextComment comment={comment} styles={styles} />}
                {editing && <EditForm
                    placeholder={commentValueEdited} value={commentValueEdited}
            ...
            <div className={styles.actions_comment}>
                <ActionsComment
                    commentOrReply={comment} handleEdit={handleEdit} handleRemove={handleRemove}
            ...
            {(replying || (arrayReplies && arrayReplies.length > 0) || showReplies || replies) && <>
                <div className={styles.dynamic_itens}>
                    {replying && <ReplyForm
                        commentOrReply={comment} quizId={quizId}
                    ...
                    {((arrayReplies && arrayReplies.length > 0) || (replies && replies.length > 0)) && <>
                        <span className={`${styles.show_more} ${showReplies ? styles.show_active : ''}`} onClick={handleShowReplies}>
                            <p>{t('body.showReplies', { count: replies ? replies.length : arrayReplies ? arrayReplies.length : 0 })}</p>
                            <ArrowSvg />
                        </span>
                        {showReplies && <div className={styles.replies}>
                            {replies?.map((rep, i) => (
                                <ReplyContainer key={i} quizId={quizId} reply={rep} styles={styles} commentId={comment.commentId} />
                            ))}
                        </div>}
                    </>}
                </div>
